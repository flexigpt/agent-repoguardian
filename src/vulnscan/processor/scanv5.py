...
from ..prompts.templates import VULN_SCAN_CODE_V5_BLOCK_ISSUE_ANALYSIS_PROMPT
from .data_fetch import CWE_ALL_FUNCTIONAL_CATEGORIES
from .data_fetch import CWE_ALL_FUNCTIONAL_DETAILS


def initialize_scan_response_v5(scan_item: scan_schema.ScanItem,
...
def deep_scan_on_labels(model_obj: ModelInfo, labeled_code_blocks: scan_schema.CodeBlockListV4, all_lines: List[str],
                        prompt_template: str) -> Optional[scan_schema.VulnerabilityBatchResponseV4]:
    try:
        ...
        block_str = json.dumps(all_blocks, indent=2)
        prompt_kwargs = {
        ...
        return scan_schema.VulnerabilityBatchResponseV4.model_validate({
            "code_block": {
                "categorized_lines": all_blocks
            },
...
def process_batch(model_obj: ModelInfo,
                  batch: List[code_schema.CodeItem]) -> Optional[scan_schema.VulnerabilityBatchResponseV4]:
    ...
    deep_scan_result = deep_scan_on_labels(model_obj, labeled_code_blocks, all_lines,
...
def do_vuln_scan_v5(model_obj: ModelInfo, scan_item: scan_schema.ScanItem, scan_type: scan_schema.ScanType,
                    batches: List[List[code_schema.CodeItem]]) -> Optional[scan_schema.VulnerabilityScanResponseV4]:

    scan_response = initialize_scan_response_v5(scan_item, scan_type)
    for batch in batches:
        deep_scan_result = process_batch(model_obj, batch)
        if deep_scan_result:
            scan_response.scan_responses.append(deep_scan_result)
